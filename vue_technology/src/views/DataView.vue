  ...
  <div class="real-time-data-container">
    ...
    <div class="kpi-cards">
      <el-card 
        ...
        :key="index"
        :class="['kpi-card', `kpi-card-${index % 3}`]"
      >
        <template #header>
          <div class="kpi-header">
            <div class="kpi-title">
              <el-icon :class="kpi.iconClass"><component :is="kpi.icon" /></el-icon>
              <span>{{ kpi.name }}</span>
            </div>
            <el-button 
              size="small" 
              @click="openChartDialog(index)"
              :icon="Histogram"
            >
              查看趋势
            </el-button>
        ...
        <el-row :gutter="20">
          <el-col :span="8">
            <el-form-item label="工序">
              <el-select v-model="queryParams.workstation" placeholder="请选择工序" clearable>
                <el-option
    ...
    <el-dialog v-model="reportDialogVisible" title="生成报表" width="50%">
      <el-form :model="reportForm" label-width="100px">
        ...
        <el-form-item label="报表格式" required>
          <el-radio-group v-model="reportForm.format">
            <el-radio label="csv">PDF</el-radio>
    ...
    <div class="main-chart-container">
      <div id="chart" class="main-chart">历史趋势分析</div>
    </div>
  </div>
...
<script setup>
import { getCurrentInstance, ref, onMounted, markRaw,nextTick } from 'vue'
import { ArrowRight, Lightning, Sunny, Histogram } from '@element-plus/icons-vue'
...
const kpiList = ref([
  {
    ...
    value: '5.6',
    unit: 'Pa',
    icon: markRaw(Lightning),  // 使用 markRaw
    iconClass: 'vibration-icon',
    updateTime: '10:30:25',
    chartData: generateChartData('振动', 5, 10)
  },
  {
    name: '电流数据',
    value: '1.245',
    unit: 'A',
    icon: markRaw(Sunny),  // 使用 markRaw
    iconClass: 'current-icon',
    updateTime: '10:30:22',
    chartData: generateChartData('电流', 1, 1.5)
  },
  {
    name: '温度数据',
    value: '27',
    ...
    iconClass: 'temp-icon',
    updateTime: '10:30:20',
    chartData: generateChartData('温度', 20, 30)
  }
])
...
// 生成模拟图表数据
function generateChartData(name, min, max) {
  const hours = []
  const values = []
  for (let i = 0; i < 24; i++) {
    hours.push(`${i}:00`)
...
// 打开图表对话框
function openChartDialog(index) {
  currentKpi.value = kpiList.value[index]
  dialogTitle.value = `${currentKpi.value.name}趋势图`
  chartStats.value = currentKpi.value.chartData.stats
...
function initChart() {
  ...
  const option = {
    tooltip: {
      trigger: 'axis',
      formatter: '{b0}<br/>{a0}: {c0} ' + currentKpi.value.unit
    },
    grid: {
      left: '3%',
      right: '4%',
    ...
    xAxis: {
      type: 'category',
      data: currentKpi.value.chartData.hours,
      axisLine: {
        lineStyle: {
          color: '#999'
        }
    ...
    series: [
      {
        ...
        type: 'line',
        data: currentKpi.value.chartData.values,
        smooth: true,
        lineStyle: {
          width: 3,
          color: getLineColor()
        ...
        areaStyle: {
          color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
            { offset: 0, color: getAreaColor(0.8) },
...
function getLineColor() {
  const colors = ['#6edfe7', '#f6c659', '#f56c6c']
  const index = kpiList.value.findIndex(kpi => kpi.name === currentKpi.value.name)
  return colors[index % 3]
}

// 获取区域颜色
function getAreaColor(opacity) {
  const colors = ['rgba(110, 223, 231', 'rgba(246, 198, 89', 'rgba(245, 108, 108']
  const index = kpiList.value.findIndex(kpi => kpi.name === currentKpi.value.name)
  return `${colors[index % 3]}, ${opacity})`
}
...
const workstationOptions = [
  { value: 'station1', label: '工序1' },
  { value: 'station2', label: '工序2' },
  { value: 'station3', label: '工序3' }
]
...
// 取收盘价（原来 rawData[i][2] 是收盘价）
const closePrices = rawData.map(function (item) {
  return +item[2];
});
...
.kpi-card-0 {
  border-top: 4px solid #6edfe7;
}

.kpi-card-1 {
  border-top: 4px solid #f6c659;
}
